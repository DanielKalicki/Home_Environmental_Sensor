...
/// SCD41 attached to a blocking esp-hal I2C bus.
    ...
    T: Instance,
{
    ...
    pub fn new(i2c: &'a mut I2C<'d, T, Blocking>, delay: Delay) -> Self {
        Self::with_address(i2c, delay, DEFAULT_ADDRESS)
    }

    /// Bind the driver to a bus using an explicit 7-bit address.
    pub fn with_address(i2c: &'a mut I2C<'d, T, Blocking>, delay: Delay, address: u8) -> Self {
        Self {
            i2c,
            delay,
            address,
    ...
    fn send_command(&mut self, command: u16, execution_delay_ms: u32) -> Result<(), Error> {
        self.i2c.write(self.address, &command.to_be_bytes())?;
        self.delay.delay_millis(execution_delay_ms);
        Ok(())
    }
    ...
    fn read_three_words(
        ...
        self.send_command(command, execution_delay_ms)?;
    ...
    pub fn stop_periodic_measurement(&mut self) -> Result<(), Error> {
        self.send_command(
    ...
    pub fn reinit(&mut self) -> Result<(), Error> {
        self.send_command(CMD_REINIT, REINIT_DELAY_MS)
    }
    /// Leave sleep mode.
    ///
    ...
    pub fn wake_up(&mut self) -> Result<(), Error> {
        ...
        self.delay.delay_millis(WAKE_UP_DELAY_MS);
        result.map_err(Error::from)
    }

    /// Read the 48-bit unique serial number; useful as a presence check.
    pub fn serial_number(&mut self) -> Result<u64, Error> {
        let words = self.read_three_words(CMD_GET_SERIAL_NUMBER, SHORT_COMMAND_DELAY_MS)?;
    ...
    pub fn read_measurement(&mut self) -> Result<Measurement, Error> {
        let words = self.read_three_words(CMD_READ_MEASUREMENT, SHORT_COMMAND_DELAY_MS)?;
    ...
    ///
    /// This blocks for roughly 5 seconds while the sensor converts.
    pub fn measure_single_shot(&mut self) -> Result<Measurement, Error> {
    ...
    pub fn measure_single_shot_rht_only(&mut self) -> Result<Measurement, Error> {
        self.send_command(
